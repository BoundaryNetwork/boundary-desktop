...
- **基座持有共享与框架状态。** 跨模块、框架级的有状态资源（登录态、注册表、窗口、共享连接）只存在于基座。
- **模块可持有自己的私有功能子系统，但须自洽回收。** 模块自身功能所需的资源（如浏览器模块的页面句柄 / CDP 会话 / 自动化运行）由模块自己持有并管理；对外能力一律经 `ctx.registerTool` 暴露，框架只把分配的 UI 区域交给它。deactivate 时模块彻底销毁这些资源。检验标准不变：热拔掉不留痕迹，插回去行为一致。
